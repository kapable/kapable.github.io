...
class Result extends Component {
    ...
    otherTestBannerRenderer() {
        ...
        let colorPaletteList = ["colorPalette"];
        ...
        if(personalColor2022ListKor.includes(this.state.current_test)){
            ...
            const rederTestList = ["bubbleCharEng", "alienCharEng", "loveLotteryEng", "loveTVEng", "rainDropEng",];
            return(
                <Fragment>
        ...
        } else if (bubbleCharEngList.includes(this.state.current_test)) {
            const rederTestList = ["alienCharEng", "loveLotteryEng", "loveTVEng", "rainDropEng", "digitalCharEng",];
            return (
                <Fragment>
                    {rederTestList.map((test) => (
                        <Fragment key={test + '-test-key'}>
        ...
        } else if (colorPaletteList.includes(this.state.current_test)) {
            const rederTestList = ["tanghulu", "bloodTransfusion", "unlockMyHeart", "beachChar", "lovePill", "charDegree",];
            return (
                <Fragment>
                    {rederTestList.map((test) => (
                        <Fragment key={test + '-test-key'}>
            ...
            const rederTestList = ["colorPalette", "tanghulu", "bloodTransfusion", "unlockMyHeart", "beachChar", "lovePill",];
            return(
                <Fragment>
                    {rederTestList.map((test) => (
                        <Fragment key={test + '-test-key'}>
            ...
            <div className='article-adCover-div-1'>
                <div className='article-adCover-div-2'>
                    <div className='article-adCover-div-3'>
                        <p style={{ margin: 0 }}><b><span style={{color:"#4185F4"}}>지금 쿠팡 방문하고</span></b></p>
                        <div><p style={{ fontSize: '1.4rem', fontWeight: "bolder" }}>2시간동안 광고 제거하기!</p></div>
                        <p style={{ fontSize: "0.7rem", width: "0.7rem", color: "white", backgroundColor: "#83b4de", position:"absolute", right:"0", top: "6rem" }}>
