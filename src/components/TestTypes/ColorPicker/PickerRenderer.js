...
const PickerRenderer = ({
  ...
  const [randomNum, setRandomNum] = useState(
    Math.floor(Math.random() * (data.squares + 1 - 1) + 1)
  );
  ...
  useEffect(() => {
    ...
    if (-0.15 >= remainingTime && data.round !== totalRound) {
      // for progress-bar
      setIsDone(true);
  ...
  const onCoupangButtonClick = useCallback(() => {
    ...
    _eventSenderGA(
      'Paging',
      `Click go-to-Coupang Button(ColorPicker${difficulty})`,
      'quiz page'
    );
    return navigate(testDomain);
  }, [difficulty, navigate, setCoupangCookie, testDomain]);
  ...
  const onCoupangCloseButtonClick = useCallback(() => {
    ...
    _eventSenderGA(
      'Closing',
      `Click Close-Coupang Button(ColorPicker${difficulty})`,
      'quiz page'
    );
    return navigate(testDomain);
  }, [setCoupangCookie, difficulty, navigate, testDomain]);
    ...
    return (
      <div className='article-adCover-div-1'>
        <div className='article-adCover-div-2'>
          ...
          <p className='result-coupang-comment' style={{ marginTop: '1rem' }}>
    ...
    (number) => {
      if (number === randomNum) {
        if (data.round === totalRound) {
          ...
          setIsLoading(true);
          setTimeout(() => {
            navigate(`${testDomain}/result/`, {
              state: { difficulty, totalTime, lang },
            });
          }, 2500);
        } else {
      ...
      return (
        <div className='picker-background'>
          <Progress
            ...
            percent={(remainingTime / data.seconds) * 100}
            showInfo={false}
          />
                ...
                <button
                  onClick={
                    startCoupangTimer
                      ...
                      : () =>
                          onButtonClick(
                            (row_num + 1) * gridMatrix.cols -
                              (gridMatrix.cols - (col_num + 1))
                          )
                  }
